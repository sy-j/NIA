...
cnt = 0
for path, dirs, files in tqdm.tqdm(os.walk(os.path.join(root, '100MB 분할 데이터(검사용)'))):
    for file in files:
        cnt += 1
...
for i in range(len(df)):
    if df['sEEPP'].iloc[i] == 0:
        ...
        for set_name in ['train']:
            print(f'  {set_name} set: ', end='')
            old_path = fpf.save_path(set_name, purpose='non-labeled sEEPP')
            new_path = fpf.save_path(set_name, purpose='non-labeled sEEPP(검사용)')
...
cnt = 0
for path, dirs, files in tqdm.tqdm(os.walk(os.path.join(root, 'non-labeled sEEPP(검사용)'))):
    for file in files:
        cnt += 1
        file_name = os.path.join(path, file)
        file_size = os.path.getsize(file_name)
